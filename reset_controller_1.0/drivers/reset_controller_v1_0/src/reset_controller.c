...
void RST_CTRL_viterbi(const u32 baseaddr)
{
	const u32 reg = RST_CTRL_get_ctrl_reg(baseaddr);
	RESET_CONTROLLER_mWriteReg(baseaddr, RESET_CONTROLLER_S00_AXI_SLV_REG0_OFFSET,
...
void RST_CTRL_interleave(const u32 baseaddr)
{
	const u32 reg = RST_CTRL_get_ctrl_reg(baseaddr);
	RESET_CONTROLLER_mWriteReg(baseaddr, RESET_CONTROLLER_S00_AXI_SLV_REG0_OFFSET,
		reg | RST_CTRL_INTERLEAVE_RESET);
	RESET_CONTROLLER_mWriteReg(baseaddr, RESET_CONTROLLER_S00_AXI_SLV_REG0_OFFSET,
		reg & ~RST_CTRL_INTERLEAVE_RESET);
}

void RST_CTRL_deinterleave(const u32 baseaddr)
{
	const u32 reg = RST_CTRL_get_ctrl_reg(baseaddr);
	RESET_CONTROLLER_mWriteReg(baseaddr, RESET_CONTROLLER_S00_AXI_SLV_REG0_OFFSET,
		reg | RST_CTRL_DEINTERLEAVE_RESET);
	RESET_CONTROLLER_mWriteReg(baseaddr, RESET_CONTROLLER_S00_AXI_SLV_REG0_OFFSET,
		reg & ~RST_CTRL_DEINTERLEAVE_RESET);
}